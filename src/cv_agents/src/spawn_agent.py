...
def get_path_msg(path, opt_ind):
	...
	for i in range(len(path)) :
		...
		for j in range(len(path[i].x)) :
			line_point = Point()
			line_point.x = path[i].x[j]
			line_point.y = path[i].y[j]
			line_point.z = 0.0
...
if __name__ == "__main__":
	...
	f_path_pub = rospy.Publisher("visualization_marker_array", MarkerArray, queue_size =1)
